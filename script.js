...
function hideAll() {
  ...
  // Also hide help modal and admin panel for clean dashboard
  document.getElementById("helpModal").classList.remove("show");
  document.getElementById("adminPanel").classList.add("hidden");
  // Don't hide reports list - buttons need to be able to show it
}
...
function saveReport(report) {
  ...
  // Check report limit for regular users (not admin)
  if (!currentUser.isAdmin) {
    const reports = JSON.parse(localStorage.getItem('reports') || '[]');
    const userReports = reports.filter(r => r.email === currentUser.email);
    
    if (userReports.length >= 5) {
      alert('You have reached the maximum limit of 5 reports. Please delete an existing report or contact admin for assistance.');
      return;
    }
  }
...
function login() {
  ...
  if (isValid) {
    ...
    if (user) {
      ...
      document.getElementById("adminPanelBtn").classList.add("hidden"); // Fix: add hidden class
      document.getElementById("loginPage").classList.add("hidden");
      document.getElementById("dashboard").classList.remove("hidden");
      hideAll(); // Hide all forms to show clean dashboard
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (currentUser) {
    ...
    document.getElementById("loginPage").classList.add("hidden");
    document.getElementById("dashboard").classList.remove("hidden");
    hideAll(); // Hide all forms to show clean dashboard first
  }
...
function openEditModal(index) {
  ...
  // Handle Lost-specific fields
  if (report.type === 'Lost') {
    document.getElementById('editDateLabel').style.display = 'block';
    document.getElementById('editDate').style.display = 'block';
    document.getElementById('editDate').value = report.date || '';
    
    document.getElementById('editRewardLabel').style.display = 'block';
    document.getElementById('editReward').style.display = 'block';
    document.getElementById('editReward').value = report.reward || '';
  } else {
    document.getElementById('editDateLabel').style.display = 'none';
    document.getElementById('editDate').style.display = 'none';
    document.getElementById('editRewardLabel').style.display = 'none';
    document.getElementById('editReward').style.display = 'none';
  }
...
function saveEditedReport() {
  ...
  const updatedReport = {
    ...
    items: document.getElementById('editItems').value.trim(),
    description: document.getElementById('editDescription').value.trim()
  };
  
  // Add Lost-specific fields if applicable
  if (updatedReport.type === 'Lost') {
    updatedReport.date = document.getElementById('editDate').value;
    updatedReport.reward = document.getElementById('editReward').value.trim() || null;
  }
  
  // Validate inputs
  if (!updatedReport.name || !updatedReport.email || !updatedReport.phone || 
      !updatedReport.site || !updatedReport.location || !updatedReport.items || !updatedReport.description) {
  ...
  // Email validation
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(updatedReport.email)) {
  ...
  // Phone validation
  const phoneRegex = /^[\d\s\-\+\(\)]+$/;
  if (!phoneRegex.test(updatedReport.phone) || updatedReport.phone.length < 10) {
...
function submitLostReport() {
  ...
  const fileInput = document.getElementById('lostFile');
  ...
  if (fileInput.files && fileInput.files[0]) {
    ...
    reader.onload = function(e) {
      imageData = e.target.result;
      saveLostReportWithData(imageData, itemName, name, email, phone, selectedLostSite, location, date, description, reward);
    };
    reader.readAsDataURL(fileInput.files[0]);
  } else {
    saveLostReportWithData(null, itemName, name, email, phone, selectedLostSite, location, date, description, reward);
  }
}

function saveLostReportWithData(imageData, itemName, name, email, phone, site, location, date, description, reward) {
  const report = {
    type: 'Lost',
    name: name,
    email: email,
  ...
  saveReport(report);
  alert('Lost report submitted successfully! Location: ' + site + ' - ' + location);
  goBack();
  showReports();
}
